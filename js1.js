...
const punterPanelHeightBelowDispensers = 31;
...
class Puzzle {
	constructor(puzzleSpec) {
		...
		for (let i = 0; i < tileCodes.length; i++) this.tileColours[tileCodes[i]] = colourCodeLookUp[puzzleSpec.colourSpec[i]];
	...
	refresh() {
		for (let t = 0; t < this.container.length; t++) {
			if (this.container[t] == null) {
				...
				this.tileRefs[t].style.backgroundColor = this.container[t].colour;
			}
		}
	}
...
/*		
		...
		for (let s = 1; s <= 9; s++) {
			...
			//this.squareDigits.push(this.puzzle.matrixSpec[s - 1]);
			this.squareTiles.push(null);
		}

		this.fillOrder = [];
		for (let i = 0; i < this.puzzle.matrixSpec.length; i++) {
			this.fillOrder[this.puzzle.matrixSpec[i] - 1] = i + 1;
		}
	...
	getColourSequence() {
		let colourSequence = [];
		for (let s = 1; s <= 9; s++) {
			if (this.squareTiles[s] == null) colourSequence.push("none"); else colourSequence.push(this.squareTiles[s].colour);
		}
		return colourSequence;
	}
	...
	refresh() {
		for (let s = 1; s <= 9; s++) {
			...
			if (this.squareTiles[s] == null) {
				...
				squareRef.style.visibility = `visible`;
			}
			else {
				//squareRef.style.backgroundColor = this.colourCodeLookUp[this.squareTiles[s].colour];
				squareRef.style.backgroundColor = this.squareTiles[s].colour;
				squareRef.style.visibility = `hidden`;
			}
		}
	}
/*	
